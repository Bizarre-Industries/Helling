...
This repo ships project-shared agent tooling for both Codex and Claude Code.
...
Per-contributor overrides go in `.codex/settings.local.json` and
...
- `/replan-mark <tag>` — advance both `.codex/.last-shipped-tag` and
  `.claude/.last-shipped-tag` after a next-version plan is rewritten
...
Codex loads project skills from `.agents/skills/` in this workspace. The ECC
autoskills tool also installs there. To deploy into Claude Code's checked-in
load path:
...
with the OpenAI API, ChatGPT Apps SDK, Codex, or related docs.

### Hooks (`.codex/hooks/` and `.claude/hooks/`)

Wired in `.codex/hooks.json` and `.claude/settings.json`:
...
### Council Subagents (`.codex/agents/` and `.claude/agents/`)
...
See `.codex/agents/README.md` and `.claude/agents/README.md` for deliberation
flow. Keep Codex and Claude agent descriptions semantically identical when
changing trigger lists.

...
func TestClient_DoBearerAndRefreshCookieRotation(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		http.SetCookie(w, &http.Cookie{Name: "helling_refresh", Value: "rotated"})
		_, _ = w.Write([]byte(`{"ok":true}`))
	}))
	t.Cleanup(srv.Close)

	prof := config.Profile{API: srv.URL, AccessToken: "jwt.x", RefreshCookie: "helling_refresh=old"}
	...
	if sawCookie != "helling_refresh=old" {
		t.Errorf("cookie = %q", sawCookie)
	}
	if c.RefreshCookie() != "helling_refresh=rotated" {
